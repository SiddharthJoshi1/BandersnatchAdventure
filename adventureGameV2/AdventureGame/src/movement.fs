module Movement 
    ...
    let newEnemy randNum enemyObj:enemy =
        match randNum with 
        | 1  when enemyObj.current_y > 0 ->  {enemyObj with current_y = enemyObj.current_y - squareSize; Dir = "N"} 
        | 2  when  enemyObj.current_y + squareSize < squareSizeSquared -> {enemyObj with current_y = enemyObj.current_y + squareSize; Dir = "S"}
        | 3   when  enemyObj.current_x > 0 -> {enemyObj with current_x = enemyObj.current_x - squareSize; Dir = "W"} 
        | 4  when  enemyObj.current_x + squareSize < squareSizeSquared ->  {enemyObj with current_x = enemyObj.current_x + squareSize; Dir = "E"}   
        | _ -> enemyObj
...
//Enemy stuff ends   
    ...
    let render (box: movableBox) (enemyObj:enemy) (itemList:filledTile List) (hazardList: filledTile List) (wallList: filledTile List) =
        ...
        let lst = ["dfPotion"; "atkPotion"; "hpPotion";]
    ...
    let enemy1 = {current_x = 300; current_y = 300; Dir=""}
